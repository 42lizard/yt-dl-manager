...
def setup_argument_parser():
    ...
    parser = argparse.ArgumentParser(
        description=_("yt-dl-manager: A tool for managing youtube-dl downloads.")
    )
    subparsers = parser.add_subparsers(dest="command")

    # init command
    init_parser = subparsers.add_parser("init", help=_("Create default config file."))
    ...
    # add command
    add_parser = subparsers.add_parser("add", help=_("Add a video to the queue."))
    ...
    # language command
    lang_parser = subparsers.add_parser("language", help=_("Manage language settings."))
    lang_subparsers = lang_parser.add_subparsers(dest="lang_action")

    # language set
    set_parser = lang_subparsers.add_parser("set", help=_("Set language preference."))
    ...
    # language show
    lang_subparsers.add_parser("show", help=_("Show current language setting."))
...
def _setup_maintenance_commands(subparsers):
    ...
    # list command
    list_parser = subparsers.add_parser("list", help=_("List downloads by status."))
    ...
    downloaded_parser.add_argument("--extractor", help=_("Filter by extractor type."))

    # status command
    subparsers.add_parser("status", help=_("Show queue status dashboard."))

    # remove command
    remove_parser = subparsers.add_parser("remove", help=_("Remove items from queue."))
    ...
    # export command
    export_parser = subparsers.add_parser("export", help=_("Export queue data."))
...
def main():
    ...
    # Set up logging
    log_level = getattr(
        __import__("logging"), os.getenv("YT_DL_MANAGER_LOG_LEVEL", "INFO").upper()
    )
...
def handle_language_command(args):
    ...
    if args.lang_action == "set":
        ...
        if pref is None:
            print(
                _("Language preference: automatic (currently: {})").format(current_lang)
            )
        else:
            print(
                _("Language preference: {} (currently: {})").format(pref, current_lang)
            )
...
def handle_list_command(args):
    ...
    # Map sort options
    sort_mapping = {"date": "timestamp_requested", "retries": "retries", "url": "url"}
